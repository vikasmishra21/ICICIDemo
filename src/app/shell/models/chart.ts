...
export class Chart {
  ...
  onConfigReady(output: TableOutput[], config: ChartProviderConfiguration): ChartProviderConfiguration {
    return typeof this.configReadyCallback === 'function' ? this.configReadyCallback(output, config) : config;
  }
  ...
  onTableDataReady(output: TableOutput[], dataTable: DataTable): void {
    if (typeof this.tableDataCallback === 'function') {
      this.tableDataCallback(output, dataTable);
    }
  }
