...
export class FilterService {
  ...
  hideFilter(variable: string) {
    ...
    this.optionSelectionCallback.next(new Map([[variable, new Array<FilterOption>()]]));
  }
  ...
  setDefaultChoice(variable: string, choices: FilterOption[]) {
    ...
    if (totalDefaults.every(value => this.selectedChoices.has(value))) {
      this.optionSelectionCallback.next(new Map([[variable, this.selectedChoices.get(variable)]]));
    }
  }
  ...
  removeChoice(variable: string, choice: FilterOption) {
    ...
    this.selectedChoices.set(variable, this.selectedChoices.get(variable).filter(val => val.code !== choice.code));
    this.optionSelectionCallback.next(new Map([[variable, this.selectedChoices.get(variable)]]));
  }
